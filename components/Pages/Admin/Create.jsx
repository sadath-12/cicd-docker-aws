...
const Create = ({ user }) => {
    ...
    const handleCreateSite = async (e) => {
        e.preventDefault();
        try {
            const res = await axios.post('/api/admin/new-website', { ...data, image }, {
                headers: {
                    authorization: `Bearer ${user?.token}`,
                },
            })
    ...
    return (
        <div className="container !max-w-[60rem] py-8 flex flex-col gap-8">
            ...
            <div className="flex flex-col gap-2">
                <label htmlFor="language">Assign User</label>
                <select required name="userId" onChange={handleChangeFormData} className='input' id="language">
                    {allUsers.map((user, i) => (
                        <option selected={user._id === data?.userId} value={user._id}>
                            {user.username}
                        </option>
                    ))}
                </select>
            ...
            <div className="flex flex-col gap-8 relative" title={!verified && 'Please verify your website first'}>
                ...
                <form onSubmit={handleCreateSite} className="flex flex-col gap-4 relative py-6" >
                    <div className="flex flex-col gap-3">
                        <h1 className='text-xl lg:text-3xl font-semibold'>Website Details</h1>
                        <div className="w-full h-1 max-w-[4rem] bg-themeColor"></div>
                    </div>
