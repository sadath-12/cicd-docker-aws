...
const Hero = ({ user }) => {
    return (
        <div className='min-h-[35em]  w-full relative overflow-hidden' >
            <video loop muted autoPlay className="absolute w-full h-full object-cover" >
                <source src='/images/hero_video.mp4' />
            </video>
            <div style={{ background: 'linear-gradient(rgba(0,0,0,0.5),rgba(0,0,0,0.5))' }} className="absolute items-center h-full flex flex-col justify-center min-h-[35em] z-20 inset-0 text-center gap-6">
                <h1 className='text-3xl lg:text-5xl font-semibold text-white max-w-[60rem]'>{user && user.token ? '' : 'Register to'} List & Grow Your Amazing Website using <span className='leading-normal'>Posts.Site.</span></h1>
