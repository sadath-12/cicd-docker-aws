        ...
        <div className='flex flex-col pb-4 rounded-md gap-4'>
            ...
            <div className="flex items-center justify-between">
                <Link href={data?.siteURL} legacyBehavior target={'_blank'}>
                    <a target={'_blank'} className="text-lg lg:text-xl font-semibold">{data?.siteURL}</a>
                </Link>
                <div className="flex gap-1 opacity-90 items-center">
                    <AiFillEye className='text-xl' />
                    <span className='font-semibold '>{data?.site_stats?.sr_traffic === 'notfound' ? 0 : data?.site_stats?.sr_traffic}</span>
                </div>
            </div>
